...
namespace Vigenere.Library
{
    public class VigenereCode
    {
        ...
        public VigenereCode(string codeWord, int salt)
        {
            ...
            this.salt = salt; //Falto asignar la variable salt
        }
        ...
        {
            StringBuilder decrypted = new StringBuilder();
            for (int i = 0; i <= encryptedPrhase.Length - 1; i++) //Falto restarle -1
            {
                ...
                decrypted.Append(decryptedValue); //Estaba añadiendo a la lista el valor equivocado
            }
            return decrypted.ToString();
        }
        /// <summary>
        ...
        {
            int result = 0; //Este contador no estaba funccionando, faltaba una condicion
            foreach (char c in fullAlphabet)
            {
                if (c.ToString() != letter.ToUpper()) //Como el fullAlphabet esta en mayuscula, convierto la letra a mayuscula tambien para trabajar solo con mayusculas
                {
                    result+=1;
                }
                else
